...
TYPE getMinHeap(DynArr *heap)
{
	...
	return getDynArr(heap, 0);  // Zed reps top of structure
}
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if (right < max)
	{
	   ...
	   if (compare(heap->data[min], heap->data[pos]) == -1)
	   {
...
void removeMinHeap(DynArr *heap)
{
	...
	int index = sizeDynArr(heap) - 1;

	putDynArr(heap, 0, getDynArr(heap, index));
	removeAtDynArr(heap, index);
...
void _buildHeap(DynArr *heap)
{
	...
	for (int i = (large / 2)-1; i >= 0; i--)
        {
	   _adjustHeap(heap, large, i);
	}
