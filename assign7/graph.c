...
int DFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	struct cirListDeque * newDeq = malloc(sizeof(struct cirListDeque));
	initCirListDeque(newDeq); //initialize

	// Prepare Graph
	clearVisited(g);

	addBackCirListDeque(newDeq, source);
	
	while (!isEmptyCirListDeque(newDeq))
	{
	   Vertex * vert = backCirListDeque(newDeq);
	   removeBackCirListDeque(newDeq);
	    ...
	    for (int i = 0; i < vert->numNeighbors; i++)
	   {
	      ...
	      {
	         addFrontCirListDeque(newDeq, vert->neighbors[i]);
	      }
	   }
	}
...
int BFS(Graph* g, Vertex* source, Vertex* destination)
{
	...
	addFrontCirListDeque(newDeq, vert);

	while (!isEmptyCirListDeque(newDeq))
	{
	   vert = backCirListDeque(newDeq);
	   removeBackCirListDeque(newDeq);
